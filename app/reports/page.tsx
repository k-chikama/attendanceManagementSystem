...
export default function ReportsPage() {
  ...
  // Format work time for chart
  const formatWorkTimeForChart = (minutes: number) => {
  ...
  const getDailyWorkTimeData = () => {
    const chartData = filteredRecords
      .map((record) => {
        const date = record.date.split("-")[2];
        return {
          date: `${date}日`,
          workTime: record.totalWorkTime
            ? Number((record.totalWorkTime / 60).toFixed(1))
            : 0,
          breakTime: record.totalWorkTime
            ? Number((record.totalWorkTime / 60).toFixed(1))
            : 0,
        };
      })
  ...
  const records = filteredRecords.map((record) => ({
    ...
    breakTime: record.totalWorkTime ? record.totalWorkTime / 60 : 0, // Convert to hours
  }));

  // Calculate totals
  const workDays = filteredRecords.filter(
    (record) => record.status === "present" || record.status === "late"
  ).length;
  const totalWorkTime = filteredRecords.reduce(
    (sum, record) => sum + (record.totalWorkTime || 0),
    0
  );
  const totalBreakTime = filteredRecords.reduce(
    (sum, record) => sum + (record.totalWorkTime || 0),
    0
  );
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <CardContent>
              ...
              <div className="text-sm text-muted-foreground">
                平均:{" "}
                {formatWorkTime(workDays > 0 ? totalWorkTime / workDays : 0)} /
                日
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="text-sm text-muted-foreground">
                平均:{" "}
                {formatWorkTime(workDays > 0 ? totalBreakTime / workDays : 0)} /
                日
              </div>
            </CardContent>
          </Card>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between border-b py-1">
                        ...
                        <span className="font-medium">
                          {
                            filteredRecords.filter((r) => r.status === "leave")
                              .length
                          }{" "}
                          日
                        </span>
                      ...
                      <div className="flex items-center justify-between border-b py-1">
                        ...
                        <span className="font-medium">
                          {
                            filteredRecords.filter((r) => r.status === "late")
                              .length
                          }{" "}
                          日
                        </span>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between border-b py-1">
                        ...
                        <span className="font-medium">
                          {formatWorkTime(averageWorkTime)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between border-b py-1">
                        <span className="text-muted-foreground">
                        ...
                        <span className="font-medium">
                          {filteredRecords.length > 0
                            ? (() => {
                                const maxRecord = [...filteredRecords].sort(
                                  (a, b) =>
                                    (b.totalWorkTime || 0) -
                                    (a.totalWorkTime || 0)
                                )[0];
                                return `${
                                  maxRecord.date.split("-")[2]
                                }日 (${formatWorkTime(
                                  maxRecord.totalWorkTime
                                )})`;
                              })()
                            : "-"}
                        </span>
